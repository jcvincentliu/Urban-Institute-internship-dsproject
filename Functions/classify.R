...
## The classify function classify a nonprofit by the organization's expenditure
## level (only if type = "pc") and NTEE major group. The function takes in a data and return a tibble that has gone
## through the transformation. The function should be used in conjuncture with the 
## process_corefile function in `Process NCCS Core Files.R`. 
...
classify <- function(dataset, type) {
  
  type = toupper(type)
  
  if (!type %in% c("PC", "PF", "CO")) {
